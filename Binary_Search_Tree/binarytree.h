...
public:
   ...
   */
  virtual TreeNode *find(const int value) { return _findSubtree(value, _root); }

  /**@param value The value of the needed Node
   * @return The distance of the first found node with the given Value to the
   * root.*/

  virtual int level(const int value) const {
    return _findLevel(_root, value, 0);
  }

  /** @return The minimum value in the BinaryTree. */
  virtual int min() const { return _minSubtree(_root); }

  /** @return The maximum value in the BinaryTree. */
  virtual int max() const { return _maxSubtree(_root); }
  ...
  virtual BinaryTree *clone() const { return new BinaryTree(*this); }
