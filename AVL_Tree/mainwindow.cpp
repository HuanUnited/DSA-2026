...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), _tree(std::make_unique<BinaryTree>()),
      _tree2(std::make_unique<BinaryTree>()) {
...
void MainWindow::_buildUi() {
  ...
  // --- Tree Type Selector -----------------------------------------------
  {
    QGroupBox *box = new QGroupBox("Tree Mode");
    QVBoxLayout *layout = new QVBoxLayout(box);
    _treeTypeSelector = new QComboBox;
    _treeTypeSelector->addItem("Binary Tree (Randomized)");
    _treeTypeSelector->addItem("Binary Search Tree");
    _treeTypeSelector->addItem("AVL Tree (Self-Balancing)");
    layout->addWidget(_treeTypeSelector);
    leftLayout->addWidget(box);

    connect(_treeTypeSelector, SIGNAL(currentIndexChanged(int)), this,
            SLOT(on_treeTypeChanged(int)));
  }
  ...
  // Mutators
  {
    ...
    grid->setSpacing(6);
    grid->addWidget(_makeButton("Add", "#27ae60", SLOT(on_addButton_clicked())),
                    0, 0);
    grid->addWidget(
        _makeButton("Remove", "#e74c3c", SLOT(on_removeButton_clicked())), 0,
        1);
    grid->addWidget(_makeButton("Remove Subtree", "#c0392b",
                                SLOT(on_removeSubtreeButton_clicked())),
                    1, 0);
    grid->addWidget(
        _makeButton("Clear Tree", "#7f8c8d", SLOT(on_clearButton_clicked())), 1,
        1);
    leftLayout->addWidget(box);
  }
  ...
  // Queries
  {
    ...
    grid->setSpacing(6);
    grid->addWidget(
        _makeButton("Find", "#3a7bd5", SLOT(on_findButton_clicked())), 0, 0);
    grid->addWidget(
        _makeButton("Level", "#3a7bd5", SLOT(on_levelButton_clicked())), 0, 1);
    grid->addWidget(
        _makeButton("Height", "#8e44ad", SLOT(on_heightButton_clicked())), 1,
        0);
    grid->addWidget(
        _makeButton("Size", "#8e44ad", SLOT(on_sizeButton_clicked())), 1, 1);
    grid->addWidget(_makeButton("Min", "#16a085", SLOT(on_minButton_clicked())),
                    2, 0);
    grid->addWidget(_makeButton("Max", "#e67e22", SLOT(on_maxButton_clicked())),
                    2, 1);
    grid->addWidget(
        _makeButton("Balanced?", "#d35400", SLOT(on_balancedButton_clicked())),
        3, 0);
    grid->addWidget(
        _makeButton("Get Vector", "#d35400", SLOT(on_vectorButton_clicked())),
        3, 1);
    leftLayout->addWidget(box);
  }
  ...
  // Tree 2 / Compare
  {
    ...
    grid->setSpacing(6);
    grid->addWidget(_makeButton("Duplicate Tree 1", "#2980b9",
                                SLOT(on_duplicateButton_clicked())),
                    0, 0);
    grid->addWidget(_makeButton("Compare Trees", "#8e44ad",
                                SLOT(on_compareButton_clicked())),
                    0, 1);
    grid->addWidget(_makeButton("Clear Tree 2", "#7f8c8d",
                                SLOT(on_clearTree2Button_clicked())),
                    1, 0, 1, 2);
    vbox->addLayout(grid);
    leftLayout->addWidget(box);
  }
  ...
  // Tests
  {
    ...
    QVBoxLayout *layout = new QVBoxLayout(box);
    auto *btn = _makeButton("Run All Tests", "#1abc9c",
                            SLOT(on_runTestsButton_clicked()));
...
QPushButton *MainWindow::_makeButton(const QString &label, const QString &color,
                                     const char *slot) {
  auto *btn = new QPushButton(label, this);
  btn->setStyleSheet(QString("QPushButton { background-color: %1; }"
...
void MainWindow::_updateTrees() {
  _treeWidget1->setTree(_tree.get());
  _treeWidget2->setTree(_tree2.get());
  _treeWidget1->update();
  _treeWidget2->update();

  bool t2empty = (_tree2->size() == 0);
  _tree2StatusLabel->setText(
      t2empty ? "Tree 2: empty"
...
void MainWindow::_log(const QString &msg) { _outputDisplay->append(msg); }

void MainWindow::_logHtml(const QString &html) { _outputDisplay->append(html); }
...
void MainWindow::on_duplicateButton_clicked() {
  _tree2.reset(_tree->clone()); // Uses the virtual clone idiom
...
void MainWindow::on_compareButton_clicked() {
  if (*_tree == *_tree2) // Dereference smart pointers for operator==
    _logHtml("<span style='color:#27ae60;'>[COMPARE] Trees are EQUAL.</span>");
  else
    _logHtml(
        "<span style='color:#e74c3c;'>[COMPARE] Trees are NOT equal.</span>");
}
...
void MainWindow::on_runTestsButton_clicked() {
  ...
  QStringList sections = {"Basic Operations",        "Search and Stats",
                          "Structure and Balancing", "BFS Iterators",
                          "Vector and Operators",    "Random Tree",
                          "Binary Search Tree", "AVL Tree"};
  int secIdx = 0;
  int sectionCounts[] = {4, 6, 2, 2, 6, 5, 4, 6}; // tests per section
  int secCount = 0;
...
void MainWindow::on_clearOutputButton_clicked() { _outputDisplay->clear(); }

void MainWindow::on_treeTypeChanged(int index) {
  if (index == 0) {
    _tree = std::make_unique<BinaryTree>();
    _tree2 = std::make_unique<BinaryTree>();
    _log("[TYPE] Switched to standard Binary Tree.");
  } else if (index == 1) {
    _tree = std::make_unique<BinarySearchTree>();
    _tree2 = std::make_unique<BinarySearchTree>();
    _log("[TYPE] Switched to Binary Search Tree.");
  } else {
    _tree = std::make_unique<AvlTree>();
    _tree2 = std::make_unique<AvlTree>();
    _log("[TYPE] Switched to AVL Tree.");
  }
  _treeWidget1->clearHighlight();
  _treeWidget2->clearHighlight();
  _updateTrees();
