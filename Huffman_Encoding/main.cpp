...
TEST(build_replaces_old_tree) {
  ...
  huffmanCoder.build("aaa");
  std::string enc1, dec1;
  double r1 = huffmanCoder.encode("aaa", enc1);
  REQUIRE(r1 > 0);
  REQUIRE(huffmanCoder.decode(enc1, dec1) && dec1 == "aaa");

  // Rebuild for completely different alphabet
  huffmanCoder.build("xyz");
  std::string enc2, dec2;
  double r2 = huffmanCoder.encode("xyz", enc2);
  REQUIRE(r2 > 0);
  REQUIRE(huffmanCoder.decode(enc2, dec2) && dec2 == "xyz");

  // Old encoding must NOT decode correctly with new tree
  // (the trees are different, so the result should be wrong or fail)
  std::string bad;
  bool ok = huffmanCoder.decode(enc1, bad);
  REQUIRE(!ok || bad != "aaa");
}
